#!/usr/bin/env python3
"""
Fix LDD (Local Drain Direction) using pyflwdir for Ethiopia.
...
Also fixes N_River missing values after river network regeneration.
"""
...
ldd = np.where((ldd == 255) & mask, 5, ldd)
...
# =============================================================================
# FIX N_RIVER MISSING VALUES
# =============================================================================
print("\n" + "=" * 80)
print("STEP 5b: Fixing N_River missing values...")
print("=" * 80)

# Get existing N_River
n_river_old = ds['N_River'].values.copy()
river_mask_bool = (wflow_river == 1)

# Count NaN values at river locations
nan_count = np.sum(np.isnan(n_river_old) & river_mask_bool)
print(f"N_River NaN values at river cells: {nan_count:,}")

# Fill missing N_River with default value 0.035
n_river_fixed = n_river_old.copy()
n_river_fixed[(river_mask_bool) & (np.isnan(n_river_old))] = 0.035
n_river_fixed[~river_mask_bool] = np.nan
n_river_fixed[~mask] = np.nan

print(f"N_River range after fix: {np.nanmin(n_river_fixed):.3f} - {np.nanmax(n_river_fixed):.3f}")
...
nan_after = np.sum(np.isnan(n_river_check) & river_mask_check)
print(f"N_River NaN at river cells after fix: {nan_after}")
...
print("N_River missing values have been filled with default 0.035.")
print("This should resolve 'LDD cycles detected' and 'missing values' errors in Wflow.")
