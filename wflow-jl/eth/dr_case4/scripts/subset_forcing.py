#!/usr/bin/env python3
"""
Subset forcing to Ethiopia region only (no resampling yet).
...
# Subset (note: lat is descending in source, so reverse the slice)
ds_subset = ds_force.sel(
    lat=slice(lat_max_buf, lat_min_buf),  # Reversed for descending lat
