#!/bin/bash
# Complete workflow: Download CHIRPS, ERA5, prepare forcing.nc, and move to dr_case5

set -e

DOWNLOAD_DIR=/data/kenya_downloads
SCRIPTS_DIR=/mnt/hydromt_data/bdi_trail2/dr_case5/scripts
CASE_DIR=/mnt/hydromt_data/bdi_trail2/dr_case5

cd $DOWNLOAD_DIR

echo '=== Kenya Complete Workflow ==='
echo ''
...
if [ $CHIRPS_COUNT -lt 1461 ]; then
    ...
    echo '  Monitor with: tail -f /data/kenya_downloads/chirps_download.log'
else
...
if [ ! -f "data/era5/temperature_2m_2020_2023.nc" ] || [ ! -f "data/era5/potential_evaporation_2020_2023.nc" ]; then
    ...
    cd $SCRIPTS_DIR
    python3 02_download_era5_kenya.py || {
...
# Step 4: Move to dr_case5
echo ''
echo 'Step 4: Moving forcing.nc to dr_case5...'
