#!/usr/bin/env python3
"""
Fix LDD (Local Drain Direction) using pyflwdir for Uganda.
...
Also fixes missing N_River values at new river cells.
...
# =============================================================================
print("\n" + "=" * 80)
print("STEP 6: Fixing N_River values...")
print("=" * 80)

# Get existing N_River
N_river_old = ds['N_River'].values.copy()
river_mask_new = (wflow_river == 1)

# Count missing N_River at river cells
missing_n_river = np.sum(river_mask_new & np.isnan(N_river_old))
print(f"River cells with missing N_River: {missing_n_river}")

# Fill missing N_River with default value (0.035)
N_river = N_river_old.copy()
N_river[river_mask_new & np.isnan(N_river)] = 0.035
N_river[~river_mask_new] = np.nan
N_river[~mask] = np.nan

print(f"N_River range after fix: {np.nanmin(N_river):.4f} - {np.nanmax(N_river):.4f}")
print(f"N_River valid values: {np.sum(~np.isnan(N_river)):,}")
...
print("  - wflow_gauges")
print("  - wflow_pits")
print("  - N_River (missing values filled)")
print("  - Slope")
...
n_river_missing = np.sum(river_mask_check & np.isnan(n_river_check))
print(f"N_River missing at river cells: {n_river_missing}")
