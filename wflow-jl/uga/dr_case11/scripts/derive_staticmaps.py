#!/usr/bin/env python3
"""
Derive complete Wflow staticmaps for Uganda from 10 raw GeoTIFF inputs.
...
Uses parallel processing with 4 threads for faster computation.

Region: Uganda (dr_case11)
Drought Period: 2021-01-01 to 2022-12-31
Grid: 313 x 235 cells (~1km resolution)
"""
...
# Wflow v1.0.1 requires exactly 3 soil layers in TOML
# BUT we create 4 layers in NetCDF to work around Brooks-Corey bug
SOIL_LAYERS = [100, 300, 800]  # mm - total 1200mm (TOML config)
N_LAYERS = 4  # Create 4 layers in NetCDF (workaround)
...
# =============================================================================
# 4. CALCULATE SLOPES (PARALLEL)
# =============================================================================
print("\n" + "=" * 80)
print(f"STEP 4: Calculating surface and river slopes ({N_THREADS} threads)...")
print("=" * 80)
...
# =============================================================================
# 6. SOIL PARAMETERS FROM TEXTURE (PARALLEL PEDOTRANSFER FUNCTIONS)
# =============================================================================
print("\n" + "=" * 80)
print(f"STEP 6: Calculating soil hydraulic parameters ({N_THREADS} threads)...")
print("=" * 80)
...
# CRITICAL: Create 4 layers (workaround for Wflow v1.0.1 bug)
...
# c at 4 soil layers (workaround: create 4 layers even though only 3 used)
...
print(f"c shape: {c_layers.shape} (4 layers for v1.0.1 workaround)")

# =============================================================================
# 7. KSAT PARAMETERS AT DIFFERENT DEPTHS (PARALLEL)
# =============================================================================
print("\n" + "=" * 80)
print(f"STEP 7: Calculating Ksat at different depths ({N_THREADS} threads)...")
print("=" * 80)
...
# Ksat at different depths (parallel)
def calc_ksat_at_depth(depth):
    depth_mm = depth * 10
    ksat_at_depth = ksat_base * np.exp(-f_param * depth_mm)
    ksat_at_depth = np.clip(ksat_at_depth, 1, 5000)
    ksat_at_depth[~mask] = np.nan
    return depth, ksat_at_depth.astype(np.float32)

depths_cm = [0.0, 5.0, 15.0, 30.0, 60.0, 100.0, 200.0]
ksat_depths = {}

with ThreadPoolExecutor(max_workers=N_THREADS) as executor:
    results = list(executor.map(calc_ksat_at_depth, depths_cm))
    for depth, ksat_arr in results:
        ksat_depths[f"KsatVer_{depth}cm"] = ksat_arr
...
print(f"kv shape: {kv_layers.shape} (4 layers for v1.0.1 workaround)")

# sl layers (4 soil layer thicknesses)
sl_layers = np.zeros((N_LAYERS, ny, nx), dtype=np.float64)
layer_thicknesses = [100, 300, 800, 0]  # 4th layer is dummy (0mm)
for i, thickness in enumerate(layer_thicknesses):
    sl_layers[i] = thickness
    sl_layers[i][~mask] = np.nan

# =============================================================================
# 8. LAND COVER LOOKUP TABLES (PARALLEL)
# =============================================================================
print("\n" + "=" * 80)
print(f"STEP 8: Creating land cover lookup tables ({N_THREADS} threads)...")
print("=" * 80)
...
# =============================================================================
# 9. LAI (MONTHLY LEAF AREA INDEX) - PARALLEL
# =============================================================================
print("\n" + "=" * 80)
print(f"STEP 9: Creating monthly LAI layers ({N_THREADS} threads)...")
print("=" * 80)

# Monthly LAI multipliers for tropical climate (Uganda - bimodal rainfall)
monthly_lai_factor = [
    0.85, 0.90, 1.00, 1.00, 0.90, 0.75,  # Jan-Jun
    0.60, 0.55, 0.70, 0.90, 0.95, 0.90,  # Jul-Dec
]
...
def calc_lai_month(month):
    ...
    return month, lai_month

LAI = np.zeros((12, ny, nx), dtype=np.float32)
with ThreadPoolExecutor(max_workers=N_THREADS) as executor:
    results = list(executor.map(calc_lai_month, range(12)))
    for month, lai_month in results:
        LAI[month] = lai_month
...
# Snow parameters (Uganda is tropical but include for completeness)
...
print(f"Soil layers: {N_LAYERS} layers in NetCDF (workaround for v1.0.1 bug)")
print(f"TOML should use: soil_layer__thickness = [100, 300, 800]")
print("=" * 80)
