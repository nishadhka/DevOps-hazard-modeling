#!/usr/bin/env python3
"""
Derive complete Wflow staticmaps for Rwanda from 10 raw GeoTIFF inputs.
This script creates all 80+ variables needed for Wflow SBM simulation.

Adapted for Wflow v1.0.1 with 3 soil layers [100, 300, 800] mm.
"""
...
SOIL_LAYERS = [100, 300, 800]  # mm - total 1200mm
N_LAYERS = 3
...
# Brooks-Corey c parameter (3 layers for Wflow v1.0.1)
...
# c at 3 soil layers (slight variation with depth)
c_layers = np.zeros((N_LAYERS, ny, nx), dtype=np.float64)
depth_factors = [1.0, 0.95, 0.90]  # 3 layers
...
# kv layers (3 layers for Wflow v1.0.1)
layer_depths_mm = [50, 250, 650]  # Midpoints for [100, 300, 800] mm layers
...
# sl layers (3 soil layer thicknesses)
sl_layers = np.zeros((N_LAYERS, ny, nx), dtype=np.float64)
for i, thickness in enumerate(SOIL_LAYERS):
...
# Monthly LAI multipliers for tropical climate (Rwanda)
monthly_lai_factor = [
    0.85, 0.90, 0.95, 1.00, 0.95, 0.80,
    0.65, 0.55, 0.65, 0.80, 0.90, 0.90,
]
...
# Snow parameters (Rwanda is tropical but include for completeness)
...
layer_coord = np.arange(1, N_LAYERS + 1)  # 1-3 for layers
...
print(f"Soil layers: {N_LAYERS} layers {SOIL_LAYERS} mm")
print("=" * 80)
