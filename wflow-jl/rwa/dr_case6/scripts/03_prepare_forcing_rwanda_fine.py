#!/usr/bin/env python3
"""
Step 3: Prepare Wflow Forcing File (forcing.nc) for Rwanda - Fine Resolution
...
print("="*70)
print("PREPARE WFLOW FORCING FILE - RWANDA (2016-2017) - FINE RESOLUTION")
print("="*70)
    ...
    for chirps_file in chunk_files:
        try:
            with rasterio.open(chirps_file) as src:
                ...
                lat_mask = (y >= RWANDA_BOUNDS['south']) & (y <= RWANDA_BOUNDS['north'])
                lon_mask = (x >= RWANDA_BOUNDS['west']) & (x <= RWANDA_BOUNDS['east'])
...
temp_ds = xr.open_dataset(ERA5_DIR / 'temperature_2m_2016_2017.nc')
temp = temp_ds['t2m']
...
pet_ds = xr.open_dataset(ERA5_DIR / 'potential_evaporation_2016_2017.nc')
pet = pet_ds['pev']
...
forcing.attrs['title'] = 'Wflow forcing - Rwanda'
