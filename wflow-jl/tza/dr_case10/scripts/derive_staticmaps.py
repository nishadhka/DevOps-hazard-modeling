#!/usr/bin/env python3
"""
Derive complete Wflow staticmaps for Tanzania from 10 raw GeoTIFF inputs.
...
Uses 4-thread parallel processing for large datasets.

Tanzania Grid: 1198 × 1248 cells (~1km resolution)
Period: 2022-01-01 to 2023-12-31 (drought event)
"""
...
DATA_DIR = os.path.join(BASE_DIR, "wflow_datasets_1km")
OUTPUT_FILE = os.path.join(BASE_DIR, "data/input/staticmaps.nc")

# Wflow v1.0.1 requires 3 soil layers BUT we create 4 layers as workaround
# for the Brooks-Corey bug (Wflow reads first 3 from 4-layer data)
SOIL_LAYERS = [100, 300, 800]  # mm - total 1200mm (what TOML uses)
N_LAYERS = 4  # Create 4 layers to avoid Brooks-Corey bug
...
def load_raster(filepath):
    """Load a raster file and return as numpy array."""
    da = rioxarray.open_rasterio(filepath).squeeze()
    return da.values.astype(np.float32), da.y.values, da.x.values

def process_chunk(func, data, chunk_size=500):
    ...
    for i in range(0, ny, chunk_size):
        for j in range(0, nx, chunk_size):
            ...
            result[i:i_end, j:j_end] = func(data[i:i_end, j:j_end])
    return result

# =============================================================================
# 1. LOAD RAW DATA (PARALLEL)
# =============================================================================
print("=" * 80)
print(f"STEP 1: Loading raw {REGION} GeoTIFF data using {N_THREADS} threads...")
print("=" * 80)

# Define file paths
files = {
    'dem': os.path.join(DATA_DIR, "1_elevation_merit_1km.tif"),
    'landuse': os.path.join(DATA_DIR, "2_landcover_esa_1km.tif"),
    'sand': os.path.join(DATA_DIR, "3_soil_sand_1km.tif"),
    'silt': os.path.join(DATA_DIR, "3_soil_silt_1km.tif"),
    'clay': os.path.join(DATA_DIR, "3_soil_clay_1km.tif"),
    'rootzone': os.path.join(DATA_DIR, "4_soil_rootzone_depth_1km.tif"),
    'ksat': os.path.join(DATA_DIR, "5_soil_ksat_1km.tif"),
    'porosity': os.path.join(DATA_DIR, "5_soil_porosity_1km.tif"),
    'flow_dir': os.path.join(DATA_DIR, "6_river_flow_direction_1km.tif"),
    'flow_acc': os.path.join(DATA_DIR, "6_river_flow_accumulation_1km.tif"),
}

# Load all files in parallel
data = {}
with ThreadPoolExecutor(max_workers=N_THREADS) as executor:
    futures = {name: executor.submit(load_raster, path) for name, path in files.items()}
    for name, future in futures.items():
        arr, lat_tmp, lon_tmp = future.result()
        data[name] = arr
        if name == 'dem':
            lat = lat_tmp
            lon = lon_tmp
...
dem_vals = data['dem']
mask = ~np.isnan(dem_vals) & (dem_vals > -500)
...
# Set masked areas to pit (5) not NaN to avoid issues
ldd[~mask] = 5

# Convert to uint8 for Wflow (fix Missing value issue)
ldd_uint8 = ldd.astype(np.uint8)
...
# Upstream area in km²
uparea = data['flow_acc'].astype(np.float32)
uparea[~mask] = np.nan
...
sand_frac = data['sand'] / 100.0
clay_frac = data['clay'] / 100.0
silt_frac = data['silt'] / 100.0
...
# c at 4 soil layers (WORKAROUND for Wflow v1.0.1 Brooks-Corey bug)
# Create 4 layers even though TOML only uses 3 - this avoids the bug
c_layers = np.zeros((N_LAYERS, ny, nx), dtype=np.float64)
...
print(f"Created {N_LAYERS} layers for Brooks-Corey workaround")
...
layer_depths_mm = [50, 250, 650, 1000]  # Midpoints for [100, 300, 800, extra] mm layers
kv_layers = np.zeros((N_LAYERS, ny, nx), dtype=np.float64)
...
sl_layers = np.zeros((N_LAYERS, ny, nx), dtype=np.float64)
layer_thicknesses = [100, 300, 800, 400]  # Extra 4th layer for workaround
...
# Monthly LAI multipliers for semi-arid climate (Tanzania)
monthly_lai_factor = [
    0.70, 0.75, 0.85, 0.95, 0.90, 0.75,
    0.60, 0.55, 0.60, 0.75, 0.85, 0.80,
]
...
# Snow parameters (Tanzania has highlands like Kilimanjaro)
...
layer_coord = np.arange(1, N_LAYERS + 1)  # 1-4 for layers (workaround)
...
# Soil properties (4 layers for workaround)
...
print(f"Soil layers: {N_LAYERS} layers (workaround for Brooks-Corey bug)")
print("TOML should still use: soil_layer__thickness = [100, 300, 800]")
print("=" * 80)
