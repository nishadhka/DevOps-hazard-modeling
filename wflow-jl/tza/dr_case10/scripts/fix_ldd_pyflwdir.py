#!/usr/bin/env python3
"""
Fix LDD (Local Drain Direction) using pyflwdir for Tanzania.
...
4. Filling missing N_River values at new river cells
5. Updating staticmaps.nc with corrected values

Input: staticmaps.nc with potentially cyclic LDD
Output: staticmaps.nc with corrected LDD and river parameters
"""
...
# Get existing N_River for filling later
N_river_original = ds['N_River'].values.copy()
...
print("Running pyflwdir.from_dem() - this may take a few minutes for large grids...")
...
# Convert value 255 (nodata/edge flow in pyflwdir) to pit (5) for valid cells
# In PCRaster LDD, 255 is invalid - should be 1-9 only
ldd_255_count = (ldd == 255).sum()
print(f"Cells with value 255 (edge/nodata flow): {ldd_255_count}")
ldd = np.where((ldd == 255) & mask, 5, ldd)  # Convert to pits within valid mask
...
# Fix any negative upstream areas (set to minimum)
negative_uparea_count = (uparea < 0).sum()
if negative_uparea_count > 0:
    print(f"WARNING: {negative_uparea_count:,} cells with negative upstream area - setting to 0.99 km²")
    uparea = np.where(uparea < 0, 0.99, uparea)
...
# =============================================================================
# FIX MISSING N_RIVER VALUES
# =============================================================================
print("\n" + "=" * 80)
print("STEP 6: Fixing missing N_River values...")
print("=" * 80)

# N_River should have values where wflow_river == 1
river_mask_bool = (wflow_river == 1)
n_river_missing = river_mask_bool & np.isnan(N_river_original)
missing_count = n_river_missing.sum()

print(f"River cells: {river_mask_bool.sum():.0f}")
print(f"N_River missing at river cells: {missing_count:.0f}")

# Fill missing N_River with default value (0.035)
N_river_fixed = N_river_original.copy()
N_river_fixed[n_river_missing] = 0.035
N_river_fixed[wflow_river != 1] = np.nan
N_river_fixed[~mask] = np.nan

print(f"Filled {missing_count:.0f} missing N_River values with 0.035")
print(f"N_River range: {np.nanmin(N_river_fixed):.3f} - {np.nanmax(N_river_fixed):.3f}")

# =============================================================================
# FIND MAIN OUTLET
# =============================================================================
print("\n" + "=" * 80)
print("STEP 7: Identifying main outlet...")
print("=" * 80)
...
if pit_mask.sum() > 0:
    ...
    wflow_pits = ds['wflow_pits'].values

# =============================================================================
# UPDATE DATASET
# =============================================================================
print("\n" + "=" * 80)
print("STEP 8: Updating staticmaps dataset...")
print("=" * 80)
...
ds['N_River'] = xr.DataArray(N_river_fixed, dims=['lat', 'lon'], attrs={'long_name': 'Manning roughness for rivers'})
...
print("  - wflow_gauges")
print("  - wflow_pits")
print("  - N_River (filled missing values)")
print("  - Slope")
...
# =============================================================================
print("\n" + "=" * 80)
print("STEP 9: Saving updated staticmaps...")
print("=" * 80)

# Store data in memory with dimensions info
data_vars = {}
var_dims = {}
for var in ds.data_vars:
    data_vars[var] = ds[var].values.copy()
    var_dims[var] = ds[var].dims
coords = {c: ds[c].values.copy() for c in ds.coords}
attrs = {var: dict(ds[var].attrs) for var in ds.data_vars}
...
backup_file = STATICMAPS_FILE.replace('.nc', '_backup.nc')
...
ds_new = xr.Dataset(coords=coords, attrs=global_attrs)
for var in data_vars:
    ds_new[var] = xr.DataArray(data_vars[var], dims=var_dims[var], attrs=attrs.get(var, {}))

# Save to temp file first
temp_file = STATICMAPS_FILE.replace('.nc', '_temp.nc')
ds_new.to_netcdf(temp_file, format='NETCDF4')
ds_new.close()

# Replace original with temp
os.replace(temp_file, STATICMAPS_FILE)
print(f"Saved to: {STATICMAPS_FILE}")
print(f"File size: {os.path.getsize(STATICMAPS_FILE) / 1e6:.1f} MB")
...
n_river_check = ds_check['N_River'].values
river_mask_check = (river_check == 1)
n_river_missing_check = river_mask_check & np.isnan(n_river_check)
print(f"N_River missing at river cells: {n_river_missing_check.sum()}")
