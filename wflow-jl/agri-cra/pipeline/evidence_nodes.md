...
| ~~`action`~~ | **REMOVED** — the action node (`ACTION_STATES` / `build_action_cpt` / `recommended_action` / `action_*` columns) was deleted. Actions are not random variables like rainfall; an action node implies the engine *prescribes* a response. Action stays with national DRM. |
| `crma_state` | Monitor / Evaluate / Assess / **Review** (4 states) | Cost-loss rule (γ = 0.20) over the `risk_level` posterior (`compute_crma_state`) — it selects an **analytical posture**, not an action | The CRMA ladder is verb-only: it characterises the risk state and the scrutiny it warrants. Under `--agri` it is computed from the `agri_risk` posterior, with `crma_state_met` kept alongside. |
| `confidence` | [0,1] | `posterior_confidence(p) = 1 − H/H_max` (entropy sharpness) | A flat posterior scores 0 regardless of which state edges ahead. Under `--agri` derived from the agri posterior; `confidence_met` retained. |
