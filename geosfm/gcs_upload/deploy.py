...
class PrefectDeployment:
    ...
    def validate_prerequisites(self):
        ...
        try:
            result = subprocess.run(['prefect', 'cloud', 'workspace', 'ls'], 
    ...
    def setup_secrets(self):
        ...
        for var_name, var_value in variables_to_create:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    logger.error(f"Failed to set variable '{var_name}': {result.stderr}")
            except Exception as e:
        ...
        for var_name, var_value in sensitive_vars:
            ...
            try:
                ...
                if result.returncode == 0:
                    logger.info(f"Sensitive variable '{var_name}' configured successfully")
                else:
                    logger.error(f"Failed to set sensitive variable '{var_name}': {result.stderr}")
            except Exception as e:
    ...
    def setup_gcs_credentials(self):
        ...
        if gcs_creds_path and Path(gcs_creds_path).exists():
            try:
                ...
                cmd = ['prefect', 'variable', 'set', 'gcs-credentials', gcs_creds_content]
                result = subprocess.run(cmd, capture_output=True, text=True)
            ...
            logger.info("For production deployment, ensure GCS credentials are set via environment variables or Prefect variables")
    ...
    def create_work_pool(self):
        ...
        try:
            ...
            result = subprocess.run(['prefect', 'work-pool', 'ls'], 
                                  capture_output=True, text=True)
    ...
    def deploy_flows(self):
        ...
        try:
            ...
            result = subprocess.run(['prefect', 'deploy', '--all'], 
                                  capture_output=True, text=True)
            ...
            if result.returncode == 0:
                ...
                for line in lines:
                    if 'View Deployment in UI:' in line:
                        logger.info(f"Deployment UI: {line.split('View Deployment in UI: ')[1]}")
