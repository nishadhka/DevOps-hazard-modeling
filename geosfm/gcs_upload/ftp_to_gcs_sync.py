...
class HydrologyDataSync:
    ...
    def load_configuration(self):
        ...
        config = {
            ...
            'gcs_prefix': self._get_config_value('GCS_PREFIX', 'gcs-prefix', 'hydrology_data'),
            'gcs_credentials': self._get_config_value('GOOGLE_APPLICATION_CREDENTIALS', 'gcs-credentials'),
    ...
    def check_file_exists_in_gcs(self, gcs_path, local_filepath):
        ...
        try:
            ...
            if blob.size != local_size:
                self.logger.debug(f"Size mismatch for {gcs_path}: local={local_size}, gcs={blob.size}")
                return False
            ...
            self.logger.warning(f"Error checking GCS file existence for {gcs_path}: {e}")
            return False
    ...
    def run_synchronization_internal(self):
        ...
        try:
            ...
            sync_results['files_discovered'] = download_result["download_count"] + download_result["failure_count"]
            ...
            uploaded, upload_failures, skipped = gcs_upload_task(self.get_config_dict(), downloaded_files)
...
def ftp_download_task(config):
    ...
    logger.info(f"Establishing FTP connection to {config['ftp_host']}:{config['ftp_port']}")
    ...
    try:
        ...
        for filename in all_files:
            if (filename.endswith(config['file_extension']) and
                any(pattern in filename for pattern in config['file_patterns'])):
        ...
        if not target_files:
            return {"downloaded_files": [], "failed_downloads": [], "download_count": 0, "failure_count": 0}
...
def gcs_upload_task(config, downloaded_files):
    ...
    logger.info(f"Uploading {len(downloaded_files)} files to GCS bucket: {config['gcs_bucket']}")
