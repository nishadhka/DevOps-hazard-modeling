#!/usr/bin/env python3
"""
RIM2D v21 — Fix missing tributary channels + culvert burn.
...
Usage:
    micromamba run -n zarrv3 python v21/run_v21_setup.py
    cd v21 && /data/rim2d/bin/RIM2D simulation_v21.def --def flex
"""
...
V21_DIR   = WORK_DIR / "v21"
V21_INPUT = V21_DIR  / "input"
V21_INPUT.mkdir(parents=True, exist_ok=True)
(V21_DIR / "output").mkdir(exist_ok=True)
...
# ── Step 1: Build v21 DEM ─────────────────────────────────────────────────
def build_dem_v21():
    print("\n" + "="*60)
    print("Step 1: Building v21 DEM")
    print("="*60)

    dem_orig, x, y = load_dem_v10()
    dem_v21 = dem_orig.copy()
    nrows, ncols = dem_v21.shape
    tr = Transformer.from_crs("EPSG:4326", "EPSG:32636", always_xy=True)

    # ── Fix 5a (from v15): Nile floodplain burn ────────────────────────────
    nile_mask = dem_orig < NILE_ELEV_THRESH
    dem_v21[nile_mask] = NILE_TARGET_ELEV
    ...
    for feat in gj["features"]:
        ...
        for lon, lat in coords:
            ...
            if 0 <= r < nrows and 0 <= c < ncols and not np.isnan(dem_orig[r, c]):
                ...
                if dem_v21[r, c] > new_elev:
                    dem_v21[r, c] = new_elev
                    total_burned_6a += 1
        ...
        if 1 < gap_rows <= MAX_GAP_ROWS:   # only bridge short gaps
            for r in range(r_f19_north, r_f18_south):
                if not np.isnan(dem_orig[r, c]):
                    if dem_v21[r, c] > NILE_TARGET_ELEV:
                        dem_v21[r, c] = NILE_TARGET_ELEV
                        gap_cells_burned += 1
    ...
    if dem_v21[CULVERT_ROW, CULVERT_COL] > culvert_invert:
        dem_v21[CULVERT_ROW, CULVERT_COL] = culvert_invert
    ...
    for dc in [-1, 1]:
        ...
        if 0 <= c2 < ncols and not np.isnan(dem_orig[CULVERT_ROW, c2]):
            ...
            if dem_v21[CULVERT_ROW, c2] > invert2:
                dem_v21[CULVERT_ROW, c2] = invert2

    n_total = int(np.sum(dem_v21 < dem_orig - 0.5))
    ...
    dem_for_pysheds = np.where(np.isnan(dem_v21), -9999, dem_v21).astype(np.float64)
    ...
    raised_mask = (~np.isnan(dem_filled_np)) & (dem_filled_np > dem_v21 + 0.01)
    ...
    burn_mask = (~np.isnan(dem_v21)) & (dem_v21 < dem_orig - 0.1)
    dem_after_burns[burn_mask] = dem_v21[burn_mask]
    dem_after_burns = np.where(np.isnan(dem_v21), np.nan, dem_after_burns)
    ...
    dem_final = np.where(np.isnan(dem_v21), np.nan, dem_final)
...
def copy_boundary_mask():
    ...
    dst = V21_INPUT / "fluvbound_mask_v21.nc"
...
def setup_rain_symlinks():
    rain_dir = V21_INPUT / "rain"
    rain_dir.mkdir(exist_ok=True)
    existing = list(rain_dir.glob("imerg_v21_t*.nc"))
    ...
    for i in range(1, N_RAIN + 1):
        ...
        dst = rain_dir / f"imerg_v21_t{i}.nc"
...
def regenerate_inflowlocs():
    print("\nStep 4: Re-generating inflowlocs_v21.txt from v12 NPZ cache")
    ...
    out_path = V21_INPUT / "inflowlocs_v21.txt"
...
# ── Step 5: Write simulation_v21.def ─────────────────────────────────────
def write_def_file():
    print("\nStep 5: Writing simulation_v21.def")
    out_timing = " ".join(str(t) for t in range(21600, SIM_DUR+1, 21600))
    n_out = SIM_DUR // 21600
    content = f"""\
# RIM2D v21 — Fix missing tributary channels + culvert
...
# end
"""
    out_path = V21_DIR / "simulation_v21.def"
...
def save_metadata():
    ...
    out_path = V21_INPUT / "v21_metadata.json"
    with open(out_path, "w") as f:
...
if __name__ == "__main__":
    print("="*60)
    print("RIM2D v21 Setup — Tributary channels + culvert fix")
    print("="*60)
    build_dem_v21()
    copy_boundary_mask()
    setup_rain_symlinks()
