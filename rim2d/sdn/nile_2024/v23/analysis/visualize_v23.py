#!/usr/bin/env python3
"""
v23 RIM2D output — animation + analysis
========================================
Produces:
...
Usage:
    micromamba run -n zarrv3 python v23/analysis/visualize_v23.py
"""
...
V23_INPUT = Path("/data/rim2d/nile_highres/v23/input")
OUT_DIR   = Path("/data/rim2d/nile_highres/v23/output")
VIZ_DIR   = Path("/data/rim2d/nile_highres/v23/analysis/visualizations")
...
def plot_snapshots(snaps, dem_burned, dem_orig, hillshade, extent, sites_km):
    ...
    for t, wd in snaps:
        ...
        day   = hours // 24
        hr    = hours % 24
        title = f"v23 — Aug {25+day} {hr:02d}:00 UTC  ({hours}h)"
        ...
        fig.tight_layout()
        fig.savefig(VIZ_DIR / f"v23_wd_{t:07d}.png", dpi=DPI,
...
def plot_max_depth(wd_max, dem_burned, dem_orig, hillshade, extent, sites_km):
    ...
    fig.suptitle(
        "v23 — Peak inundation | Abu Hamad, Aug 25–31 2024\n"
        "Base: v10 MERIT DEM + cor1/cor2 KML burns + pysheds 2-pass fill",
        fontsize=11, y=1.01,
    )
    fig.tight_layout()
    out = VIZ_DIR / "v23_max_depth_map.png"
...
def plot_timeseries(snaps, dem_burned, dem_orig):
    ...
    axes[0].set_title("v23 — Flood evolution (Aug 25–31 2024)\n"
    ...
    fig.tight_layout()
    out = VIZ_DIR / "v23_timeseries.png"
    fig.savefig(out, dpi=DPI, bbox_inches="tight", facecolor="white")
    plt.close(fig)
    print(f"  Saved: {out}")
...
if __name__ == "__main__":
    print("Loading DEMs ...")
    dem_burned, x_ref, y_ref = load_nc(V23_INPUT / "dem_v23.nc")
    ...
    print("Loading v23 output ...")
    wd_max, _, _ = load_nc(OUT_DIR / "nile_v23_wd_max.nc")
