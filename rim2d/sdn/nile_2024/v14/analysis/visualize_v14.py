#!/usr/bin/env python3
"""
v14 RIM2D output — animation + analysis
=========================================
...
Usage:
    micromamba run -n zarrv3 python analysis/visualize_v14.py
"""
...
OUT_DIR   = Path("/data/rim2d/nile_highres/v14/output")
...
def plot_max_depth(wd_max, dem, x, y):
    ...
    ax.set_title(
        f"v14 — Peak flood depth | Abu Hamad, Aug 25–31 2024\n"
    ...
    fig.tight_layout()
    out = VIZ_DIR / "v14_max_depth_map.png"
...
def plot_timeseries(snaps):
    ...
    axes[0].set_title("v14 — Flood evolution (Aug 25–31, 6-day window)", fontsize=10)
    ...
    fig.tight_layout()
    out = VIZ_DIR / "v14_timeseries.png"
...
def print_analysis(wd_max):
    ...
    print("\n" + "=" * 72)
    print(f"{'Metric':<35} {'v11':>8} {'v12':>8} {'v13':>8} {'v14':>8}")
    print("-" * 72)
    rows = [
        ("Max depth Culvert2 (m)",  "5.83❌", "1.58✓", "1.58", f"{wd_max[222,266]:.2f}"),
        ("Hospital wadi depth (m)", "artefact", "1.59✓", "1.59", f"{wd_max[183,281]:.2f}"),
        ("Nile exit depth (m)",     "n/a",  "3.00",  "3.00", f"{wd_max[0,354]:.2f}"),
        ("Cells flooded >0.1m",     "24,845", "TBD",  "9,834", f"{int(np.sum(wd_max>0.1)):,}"),
        ("Stream channels carved",  "No❌",   "No❌",  "Yes✓",  "Yes✓"),
        ("Correct IMERG window",    "Yes*",   "Yes*",  "No❌",   "Yes✓"),
    ]
    for row in rows:
        print(f"{row[0]:<35} {row[1]:>8} {row[2]:>8} {row[3]:>8} {row[4]:>8}")
    print("=" * 72)
    print("* v11/v12: 37-day run (full IMERG). v13: loaded July rain by mistake.")
    print()
...
if __name__ == "__main__":
    ...
    print("Loading v14 output ...")
    wd_max, _, _ = load_nc(OUT_DIR / "nile_v14_wd_max.nc")
