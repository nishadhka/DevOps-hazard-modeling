#!/usr/bin/env python3
"""
RIM2D v19 — Fix missing tributary channels + culvert burn.
...
Usage:
    micromamba run -n zarrv3 python v19/run_v19_setup.py
    cd v19 && /data/rim2d/bin/RIM2D simulation_v19.def --def flex
"""
...
V19_DIR   = WORK_DIR / "v19"
V19_INPUT = V19_DIR  / "input"
V19_INPUT.mkdir(parents=True, exist_ok=True)
(V19_DIR / "output").mkdir(exist_ok=True)
...
# ── Step 1: Build v19 DEM ─────────────────────────────────────────────────
def build_dem_v19():
    print("\n" + "="*60)
    print("Step 1: Building v19 DEM")
    print("="*60)

    dem_orig, x, y = load_dem_v10()
    dem_v19 = dem_orig.copy()
    nrows, ncols = dem_v19.shape
    tr = Transformer.from_crs("EPSG:4326", "EPSG:32636", always_xy=True)

    # ── Fix 5a (from v15): Nile floodplain burn ────────────────────────────
    nile_mask = dem_orig < NILE_ELEV_THRESH
    dem_v19[nile_mask] = NILE_TARGET_ELEV
    ...
    for feat in gj["features"]:
        ...
        for lon, lat in coords:
            ...
            if 0 <= r < nrows and 0 <= c < ncols and not np.isnan(dem_orig[r, c]):
                ...
                if dem_v19[r, c] > new_elev:
                    dem_v19[r, c] = new_elev
                    total_burned_6a += 1
    ...
    for c in gap1_cols:
        ...
        if r_f17_south > r_f22_north + 1:  # gap exists
            # Burn gap from Nile north end UP TO (not including) stream south end
            # Excludes the stream endpoint row which may be an inflow boundary cell
            for r in range(r_f22_north, r_f17_south):  # exclusive of r_f17_south
                if not np.isnan(dem_orig[r, c]):
                    if dem_v19[r, c] > NILE_TARGET_ELEV:
                        dem_v19[r, c] = NILE_TARGET_ELEV
                        gap_cells_burned += 1
    ...
    for c in gap2_cols:
        ...
        if r_f18_south > r_f19_north + 1:
            for r in range(r_f19_north, r_f18_south):  # exclusive of r_f18_south
                if not np.isnan(dem_orig[r, c]):
                    if dem_v19[r, c] > NILE_TARGET_ELEV:
                        dem_v19[r, c] = NILE_TARGET_ELEV
                        gap_cells_burned += 1
    ...
    if dem_v19[CULVERT_ROW, CULVERT_COL] > culvert_invert:
        dem_v19[CULVERT_ROW, CULVERT_COL] = culvert_invert
    ...
    for dc in [-1, 1]:
        ...
        if 0 <= c2 < ncols and not np.isnan(dem_orig[CULVERT_ROW, c2]):
            ...
            if dem_v19[CULVERT_ROW, c2] > invert2:
                dem_v19[CULVERT_ROW, c2] = invert2

    # ── Summary ───────────────────────────────────────────────────────────
    n_total = int(np.sum(dem_v19 < dem_orig - 0.5))
    print(f"\n  Total cells modified from v10 DEM: {n_total}")

    # Write DEM
    src = V10_INPUT / "dem.nc"
    dst = V19_INPUT / "dem_v19.nc"
    ...
    arr_np[~np.isnan(dem_v19)] = dem_v19[~np.isnan(dem_v19)]
...
def copy_boundary_mask():
    ...
    dst = V19_INPUT / "fluvbound_mask_v19.nc"
    shutil.copy2(src, dst)
...
def setup_rain_symlinks():
    rain_dir = V19_INPUT / "rain"
    rain_dir.mkdir(exist_ok=True)
    existing = list(rain_dir.glob("imerg_v19_t*.nc"))
    ...
    for i in range(1, N_RAIN + 1):
        ...
        dst = rain_dir / f"imerg_v19_t{i}.nc"
...
def regenerate_inflowlocs():
    print("\nStep 4: Re-generating inflowlocs_v19.txt from v12 NPZ cache")
    ...
    out_path = V19_INPUT / "inflowlocs_v19.txt"
...
# ── Step 5: Write simulation_v19.def ─────────────────────────────────────
def write_def_file():
    print("\nStep 5: Writing simulation_v19.def")
    out_timing = " ".join(str(t) for t in range(21600, SIM_DUR+1, 21600))
    n_out = SIM_DUR // 21600
    content = f"""\
# RIM2D v19 — Fix missing tributary channels + culvert
...
# end
"""
    out_path = V19_DIR / "simulation_v19.def"
...
def save_metadata():
    ...
    out_path = V19_INPUT / "v19_metadata.json"
    with open(out_path, "w") as f:
...
if __name__ == "__main__":
    print("="*60)
    print("RIM2D v19 Setup — Tributary channels + culvert fix")
    print("="*60)
    build_dem_v19()
    copy_boundary_mask()
    setup_rain_symlinks()
...
Setup complete. To run:
  cd /data/rim2d/nile_highres/v19
  export LD_LIBRARY_PATH=/data/rim2d/lib:$LD_LIBRARY_PATH
  /data/rim2d/bin/RIM2D simulation_v19.def --def flex
{'='*60}
""")
