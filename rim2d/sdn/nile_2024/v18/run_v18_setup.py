#!/usr/bin/env python3
"""
RIM2D v18 — Fix missing tributary channels + culvert burn.
...
Usage:
    micromamba run -n zarrv3 python v18/run_v18_setup.py
    cd v18 && /data/rim2d/bin/RIM2D simulation_v18.def --def flex
"""
...
V18_DIR   = WORK_DIR / "v18"
V18_INPUT = V18_DIR  / "input"
V18_INPUT.mkdir(parents=True, exist_ok=True)
(V18_DIR / "output").mkdir(exist_ok=True)
...
# ── Step 1: Build v18 DEM ─────────────────────────────────────────────────
def build_dem_v18():
    print("\n" + "="*60)
    print("Step 1: Building v18 DEM")
    print("="*60)

    dem_orig, x, y = load_dem_v10()
    dem_v18 = dem_orig.copy()
    nrows, ncols = dem_v18.shape
    tr = Transformer.from_crs("EPSG:4326", "EPSG:32636", always_xy=True)

    # ── Fix 5a (from v15): Nile floodplain burn ────────────────────────────
    nile_mask = dem_orig < NILE_ELEV_THRESH
    dem_v18[nile_mask] = NILE_TARGET_ELEV
    ...
    for feat in gj["features"]:
        ...
        for lon, lat in coords:
            ...
            if 0 <= r < nrows and 0 <= c < ncols and not np.isnan(dem_orig[r, c]):
                new_elev = max(dem_orig[r, c] - depth, NILE_TARGET_ELEV)
                if dem_v18[r, c] > new_elev:
                    dem_v18[r, c] = new_elev
                    total_burned_6a += 1
    ...
    for c in gap1_cols:
        ...
        if r_f17_south > r_f22_north + 1:  # gap exists
            ...
            # Cap at NILE_TARGET_ELEV so gap cells never become a deep sink
            for r in range(r_f22_north, r_f17_south + 1):
                if not np.isnan(dem_orig[r, c]):
                    new_elev = max(dem_orig[r, c] - BURN_GAP, NILE_TARGET_ELEV)
                    if dem_v18[r, c] > new_elev:
                        dem_v18[r, c] = new_elev
                        gap_cells_burned += 1
    ...
    if dem_v18[CULVERT_ROW, CULVERT_COL] > culvert_invert:
        dem_v18[CULVERT_ROW, CULVERT_COL] = culvert_invert
    ...
    for dc in [-1, 1]:
        ...
        if 0 <= c2 < ncols and not np.isnan(dem_orig[CULVERT_ROW, c2]):
            ...
            if dem_v18[CULVERT_ROW, c2] > invert2:
                dem_v18[CULVERT_ROW, c2] = invert2

    # ── Summary ───────────────────────────────────────────────────────────
    n_total = int(np.sum(dem_v18 < dem_orig - 0.5))
    print(f"\n  Total cells modified from v10 DEM: {n_total}")

    # Write DEM
    src = V10_INPUT / "dem.nc"
    dst = V18_INPUT / "dem_v18.nc"
    ...
    arr_np[~np.isnan(dem_v18)] = dem_v18[~np.isnan(dem_v18)]
...
def copy_boundary_mask():
    ...
    dst = V18_INPUT / "fluvbound_mask_v18.nc"
    shutil.copy2(src, dst)
...
def setup_rain_symlinks():
    rain_dir = V18_INPUT / "rain"
    rain_dir.mkdir(exist_ok=True)
    existing = list(rain_dir.glob("imerg_v18_t*.nc"))
    ...
    for i in range(1, N_RAIN + 1):
        ...
        dst = rain_dir / f"imerg_v18_t{i}.nc"
...
def regenerate_inflowlocs():
    print("\nStep 4: Re-generating inflowlocs_v18.txt from v12 NPZ cache")
    ...
    out_path = V18_INPUT / "inflowlocs_v18.txt"
...
# ── Step 5: Write simulation_v18.def ─────────────────────────────────────
def write_def_file():
    print("\nStep 5: Writing simulation_v18.def")
    out_timing = " ".join(str(t) for t in range(21600, SIM_DUR+1, 21600))
    n_out = SIM_DUR // 21600
    content = f"""\
# RIM2D v18 — Fix missing tributary channels + culvert
...
# end
"""
    out_path = V18_DIR / "simulation_v18.def"
...
def save_metadata():
    ...
    out_path = V18_INPUT / "v18_metadata.json"
    with open(out_path, "w") as f:
...
if __name__ == "__main__":
    print("="*60)
    print("RIM2D v18 Setup — Tributary channels + culvert fix")
    print("="*60)
    build_dem_v18()
    copy_boundary_mask()
    setup_rain_symlinks()
...
Setup complete. To run:
  cd /data/rim2d/nile_highres/v18
  export LD_LIBRARY_PATH=/data/rim2d/lib:$LD_LIBRARY_PATH
  /data/rim2d/bin/RIM2D simulation_v18.def --def flex
{'='*60}
""")
