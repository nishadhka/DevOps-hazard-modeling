#!/usr/bin/env python3
"""
Plot all v15 input data for verification.
  1. DEM comparison: v10 original vs v15 stream-burned, burn depth difference
  2. Boundary mask (4 inflow zones)
  3. Buildings footprint
  4. Inflow locations from inflowlocs_v15.txt (WSE timeseries)

Usage:
    micromamba run -n zarrv3 python analysis/plot_v15_inputs.py
"""
...
V15_INPUT = Path("/data/rim2d/nile_highres/v15/input")
VIZ_DIR   = Path("/data/rim2d/nile_highres/v15/analysis/visualizations")
...
def plot_dem_comparison():
    ...
    dem14, _, _ = load_nc(V15_INPUT / "dem_v15.nc")
    ...
    ax.set_title(f"v15 — Stream+Nile+Railway burned DEM\n({n_burned} cells modified from v10)",
    ...
    ax.set_title(f"DEM difference (v15 − v10)\nBurned cells: {n_burned} | Max burn: {diff.min():.1f} m",
                 fontsize=11, fontweight="bold")
    ax.set_xlabel("x (km)"); ax.set_ylabel("y (km)")

    fig.suptitle("v15 Input — DEM Verification (stream burning)", fontsize=13, fontweight="bold")
    fig.tight_layout()
    out = VIZ_DIR / "v15_input_dem_comparison.png"
...
def plot_boundary_and_buildings():
    dem14, x, y  = load_nc(V15_INPUT / "dem_v15.nc")
    mask, _, _   = load_nc(V15_INPUT / "fluvbound_mask_v15.nc")
    ...
    ax.set_title(f"v15 Fluvial Boundary Mask — {n_zones} inflow zones\n"
    ...
    fig.suptitle("v15 Input — Boundary mask and buildings", fontsize=13, fontweight="bold")
    fig.tight_layout()
    out = VIZ_DIR / "v15_input_boundary_buildings.png"
    fig.savefig(out, dpi=DPI, bbox_inches="tight", facecolor="white")
    plt.close(fig)
    print(f"  Saved: {out.name}")
...
def plot_inflow_wse():
    inflow_file = V15_INPUT / "inflowlocs_v15.txt"
    ...
    ax.set_title(f"v15 Inflow WSE timeseries — {n_cells} boundary cells × {n_steps} timesteps\n"
                 f"(WSE = DEM elevation + water depth above sill, capped at sill+1.5m)",
                 fontsize=11, fontweight="bold")
    ax.legend(fontsize=9)
    ax.grid(alpha=0.3)
    fig.tight_layout()
    out = VIZ_DIR / "v15_input_inflow_wse.png"
    fig.savefig(out, dpi=DPI, bbox_inches="tight", facecolor="white")
    plt.close(fig)
    print(f"  Saved: {out.name}")


# ── 4. Fix visualize_v15.py to use v15 DEM hillshade ─────────────────────────
def check_hillshade_mismatch():
    ...
    for ax, dem, title in [
        ...
        (axes[1], dem14, "Hillshade from v15 DEM\n(stream-burned channels visible — CORRECT for v15)"),
    ]:
    ...
    fig.suptitle("Hillshade background comparison: which DEM to use for v15 visualization?",
                 fontsize=12, fontweight="bold")
    fig.tight_layout()
    out = VIZ_DIR / "v15_input_hillshade_comparison.png"
    fig.savefig(out, dpi=DPI, bbox_inches="tight", facecolor="white")
    plt.close(fig)
    print(f"  Saved: {out.name}")
